...
6. **Rail/list drags need a tall viewport.** Short scrollable lists (e.g. the parts rail) render only a handful of rows at the default 1280×900 — a drag-and-drop test that targets a row past the visible clip silently no-ops instead of failing. Bump `page.setViewportSize` (or the test's own viewport) to ~1400px tall for any spec that drags within a list/rail.
