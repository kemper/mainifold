...
6. **Rail/list drag targets can scroll out of the default viewport.** The parts-list panel is a short scroll box at the default 1280×900 viewport (~55 px tall with a few rows) — a drag-and-drop assertion on a row past the fold silently no-ops instead of failing loudly. Set a taller viewport (e.g. 1400 px) for any spec that drags within a scrollable list/rail.
