...
test.describe('smooth paintbrush', () => {
  test('brush is the default tool with smoothing on and a detail slider', async ({ page }) => {
    ...
    // smoothing is on by default. Scope to the brush's own toggle by id (the
    // slab/shape panels carry their own "Smooth edges" toggles too).
  ...
  test('a smooth stroke survives save + reload', async ({ page }) => {
    ...
    const out = await page.evaluate(async () => {
      ...
      // reload: loadVersion re-runs the code and replays the stroke descriptor.
      pw.clearColors();
