...
- `src/reconstruct/` — mesh→code reconstruction: pure kernel ported from `scripts/inverse-cad/` (`slice2d.ts` Z-slicing + Douglas-Peucker + circle/rounded-rect fitters, `sampleMesh.ts` sampling + k-d tree, `meshDistance.ts` chamfer/hausdorff, `meshComponents.ts` weld/split, `sectionCode.ts` section→levelSet codegen, `profileMesh.ts` primitive-run profiling, `voxelDiff.ts` localized diff findings, `inscribed.ts` largest inscribed box/cylinder) running in `reconstructWorker.ts`. Powers `partwright.convertToCode()` / `evalAgainstImport()` / `profileModel()` / `compareToImport()` / `fitInscribed()`, the "Convert model to code" palette command + Tools-popover panel (`src/ui/convertToCodeModal.ts` — quality presets + step/edge/dpTol threshold controls; also offered automatically after an interactive STL import), the AI tools of the same names, and the `/reconstruct` chat command (`src/ai/reconstructPrompt.ts`, subdoc `public/ai/reconstruction.md`). Unit tests: `tests/unit/reconstruct.test.ts`; e2e: `tests/reconstruct.spec.ts`.
