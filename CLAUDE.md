...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool has no `-a`/text-mode flag** — it will silently miss or truncate matches in `main.ts` the same way. Go straight to `Bash` with `grep -an` (or `rg -a`) for any search that touches this file.
