...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool doesn't support `-a`** (a session burned a call discovering this) — go straight to `Bash` `grep -a`/`rg -a` for `main.ts`, don't try the `Grep` tool first.
