...
12. **Color export**: With color regions painted, export GLB — the file should carry vertex colors. Export 3MF — the file should include an `<m:colorgroup>` and per-triangle `pid`/`p1` attributes. The export menu also offers **3MF (Bambu — all PLA)** (`export3MFBambu`), which writes a full Bambu Studio *project* package — a production-extension `3D/3dmodel.model` carrying the `BambuStudio:3mfVersion` marker and referencing the mesh in a separate `3D/Objects/object_1.model` (per-triangle color stored in Bambu's `paint_color` segmentation, **not** `m:colorgroup`), plus `Metadata/model_settings.config` and a `Metadata/project_settings.config` declaring one Generic PLA filament per color slot. Because the package looks like a genuine Bambu project, Bambu Studio loads the declared all-PLA filaments directly instead of color-matching to a mix of PLA/ABS presets. (Painted color slot _m_ → extruder _m_+1; the base/default slot is extruder 1 and carries no `paint_color`. Use the standard **3MF** export for a portable colorgroup file that opens in any slicer.)
