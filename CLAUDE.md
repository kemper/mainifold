...
> **Verify from the angle where a defect would hide — including the underside — not just the default iso/front.** The default 4-view (front/right/top/iso) can completely hide a problem on the bottom of a model (a sole clipping through a base, a foot poking through a pedestal underside, a hollow that only opens downward). When you've changed anything near the ground/underside, **add `--view az,el` with a negative elevation to look UP at the bottom** (e.g. `--view 0,-72`), or capture several angles at once with `--view "az,el;az,el;…"` (e.g. `--view "-50,28;0,-72;90,7"` for iso + underside + side). **Garment/accessory coverage bugs are the side/back analogue of the underside trap** — a belt or armor bleeding onto a sleeve, a dress hem gap, or a skin patch poking through fabric consistently hides at front/iso and only shows from a 3/4, side, or back angle; the same figure-belt-on-arm defect shipped "fixed" three separate times before a side view finally caught it, so a garment/worn-accessory change isn't verified until you've rendered at least one non-front angle. And when a user reports a defect, **reproduce their exact repro first — the camera angle they saw it from, AND the exact code path/UI entry point they hit** (which of several near-identical import destinations, which `sleeve:'short'`-vs-`'long'` branch the subject actually takes) — fixing what you can't see from your chosen angle, or fixing a branch the subject never executes, is how a "fixed" bug ships unfixed twice.
>
...
>
> **`model:preview` renders color by default** when colors are available: in-code `api.paint.*` / colored `api.label` resolve directly, and figures whose source declares **uncolored** labels (`.label('skin')`) are colored from a **palette** — `--palette-file <json>`, else a sibling `<base>.palette.json`, else `public/catalog/palettes/<base-without-figure_>.json` (so `examples/figure_chibi_wizard.js` auto-colors; mismatched names like `figure_witch`→`witch_casting` need `--palette-file`). `--no-palette` forces the neutral normal-shaded look. `eval:models` colors the same way via a per-case `palette` (see `evals/README.md`). For the **exact** baked colors/camera (QC of a final entry), the colored catalog bake is still authoritative: with a dev server up, `node scripts/build-catalog-entry.cjs --source <file> --name x --lang manifold-js --out /tmp/x.partwright.json --palette-file <palette>` writes `/tmp/x.thumb.png` (headless — no `xvfb`; "API never appeared" just means no dev server); camera via `THUMB_AZIMUTH`/`THUMB_ELEVATION`.
>
> **Measure geometry empirically when `smoothUnion` is involved — don't trust analytic primitive extents.** A `smoothUnion` bulges the surface *past* either input primitive's bounds (a foot's real underside sat ~0.79·r below the sole centre vs the analytic 0.65·r), and `bounds()` is loose. To find a true surface position, walk `evaluate(x,y,z)` along the axis in a tiny vite-node script (`npx vite-node probe.mjs` importing `__figureTestables__` + `__testables__`) until the sign flips. Set clearances/clip planes below the *measured* value, then confirm coverage with a sample-grid check (sample where one label is solid; assert the covering label is solid there too). **Never write regression tests asserting exact geometry extents from `bounds()` — it is a conservative AABB and is wrong-by-design for offset/smoothUnion shapes. Use `evaluate()` to sample the actual surface instead.**
>
> **`genus` *rising* as `edgeLength` shrinks is the signature of a non-Lipschitz `smoothUnion`/`smin` field.** Plain `union` (min) is Lipschitz and produces clean meshes; `smin` accumulates phantom topology that worsens with resolution. When `--json` shows genus climbing as you refine, suspect a `smoothUnion` rather than under-resolution. Separately: **always mesh thin SDF features in their canonical axis-aligned frame, then transform the mesh** — a rotated SDF samples thin gaps on a diagonal grid, producing pose-dependent webbing handles regardless of resolution.
>
...
> **Parsing `model:preview --json` output? Use `npm run --silent model:preview -- …` (or invoke `node scripts/model-preview.mjs` directly).** Plain `npm run` prepends its lifecycle banner (`> partwright-cad@… model:preview`) to stdout, which breaks naive `json.load` pipelines — two independent agents burned retries on this before finding `--silent`.
...
> **`scripts/build-catalog-entry.cjs` (and `catalog-regen.cjs`) need a DEV SERVER on :5173 — not a display.** Both launch **headless** Chromium (`headless: true`), so `xvfb` is *not* required; the real prerequisite is a running `npm run dev` that the bake page loads. The classic failure is `FAIL [init]: API never appeared` (or `engine warmup timeout`) — that means **no dev server was up**, not a WebGL/display problem. Fix: start `npm run dev` in the background, wait for `http://localhost:5173/` to answer, *then* bake. Current invocation: `node scripts/build-catalog-entry.cjs --source <code.js> --name "<Name>" --lang manifold-js --out public/catalog/<entry>.partwright.json [--palette-file <palette.json> | --palette-from-existing <existing-entry.json>]`; writes `<out>`’s sibling `.thumb.png` too. Pin the tile camera with `THUMB_AZIMUTH`/`THUMB_ELEVATION` (azimuth 0=front/−Y, 90=right; elevation 0=horizon, negative=from below). `--palette-from-existing` re-applies an entry’s own `byLabel` colors when re-baking; figures that paint themselves in code (`api.paint.*`, e.g. the busts) carry no `byLabel` regions, so bake them with **no** palette flag (the in-code paint colors them on run). To re-bake the whole figure catalog, loop this per entry with the dev server up. *(History: a session wrongly concluded the bake "can't run in this container" after `xvfb-run` still printed "API never appeared" — the missing piece was always the dev server.)* **Batch catalog bakes must use a fresh Playwright page per entry** — reusing one page across `createSession` calls tears down `window.partwright` after the first bake, failing all subsequent entries with `Cannot read properties of undefined`. Scripts like `catalog-regen.cjs` and `rebake-figure-catalog.cjs` already do this; any new bake loop must too.

> **A shared builder/engine fix does not automatically refresh the baked catalog — this has bitten more than a dozen sessions.** Catalog entries are baked snapshots (mesh + thumbnail + `geometryData`) generated once from their stored `code`; editing a builder like `sdfFigure.ts` fixes every consuming entry's *editor* rendering for free the moment someone opens it (opening an entry re-runs its code), but the static `/catalog` grid thumbnail and stored `geometryData` stay stale until you explicitly re-bake, and nothing currently flags that drift. After changing a builder consumed by catalog entries, grep `public/catalog/*.partwright.json` + `examples/figure_*.js` for consumers and re-bake the affected ones — don't treat "the fix landed on `main`" as "the catalog shows it." (A standing CI "catalog freshness" check that diffs source against committed bake stats is tracked as an open gap — see the Backlog in the latest `retros/reports/`.)
...
| `/reconstruct` (alias `/convert`) | Prefill a prompt to reverse-engineer an imported mesh into editable code |
...
- `src/geometry/scatter.ts` / `deform.ts` / `sculpt.ts` / `meshSdf.ts` + `meshSdfOps.ts` — the Blender-parity mesh-shaping verbs exposed via `api.meshOps` (+ flat `api.*` aliases): `scatter` (seeded instance-on-surface), `wrapAround`/`bend`/`twist`/`taper`/`alongCurve` (auto-refining `warp` wrappers), `sculpt.grab/inflate/flatten` (falloff nudges), and `round`/`smoothWeld` (voxel-lattice signed-distance morphology + `Manifold.levelSet`, dependency-free — `meshSdf.ts` is the pure grid math: scanline rasterizer, Felzenszwalb EDT, open/close, smin, blur). Docs: `public/ai/deform.md`. Unit tests: `tests/unit/meshSdf.test.ts`, `tests/unit/meshDeform.test.ts`; e2e: `tests/deform-ops.spec.ts`.
- `src/renderer/materialSpec.ts` — pure-data viewport material presets (`api.material('brass'|…)`); recorded by the manifold-js engine like `api.paint.*`, threaded through the Worker result (`MeshResult.materialSpec`), applied by `viewport.setMaterialOverride` (MeshPhysical for clearcoat/transmission). Shading only — geometry/exports untouched; re-applies on every run since it lives in code.
- `src/export/animation.ts` — viewport video exports (`partwright.exportTurntable/exportExplode/exportParamSweep`): real-time MediaRecorder capture of the canvas; explode concatenates decomposed component meshes with eased offsets, param sweep precomputes one mesh per step then plays back.
...
- `src/reconstruct/` — mesh→code reconstruction: pure kernel ported from `scripts/inverse-cad/` (`slice2d.ts` Z-slicing + Douglas-Peucker, `sampleMesh.ts` sampling + k-d tree, `meshDistance.ts` chamfer/hausdorff, `meshComponents.ts` weld/split, `sectionCode.ts` section→levelSet codegen) running in `reconstructWorker.ts`. Powers `partwright.convertToCode()` / `evalAgainstImport()`, the "Convert model to code" palette command + Tools-popover panel (`src/ui/convertToCodeModal.ts` — quality presets + step/edge/dpTol threshold controls; also offered automatically after an interactive STL import), the AI tools of the same names, and the `/reconstruct` chat command (`src/ai/reconstructPrompt.ts`, subdoc `public/ai/reconstruction.md`). Unit tests: `tests/unit/reconstruct.test.ts`; e2e: `tests/reconstruct.spec.ts`.
...
- **Nested `claude -p` must run outside the repo directory.** When shelling out to `claude -p` (e.g. in an eval harness or judge script), always run it from `os.tmpdir()`, not from the repo root. The project's Stop hook fires for any `claude` subprocess whose `cwd` is inside the repo and replaces the model's answer with reconcile text, corrupting the output silently.
...
2. **Partial implementation never closes silently; fully-resolved issues close promptly.** If a PR merges but doesn't fully satisfy its originating issue, that issue stays **open** with a checklist of what's left — or you file an explicit follow-up issue (tick the matching box on the tracking issue / scope manifest rather than closing the umbrella). Only close a source issue when **every** acceptance criterion is actually met; a merged PR is not automatically a completed issue. Conversely, when an issue IS fully satisfied, close it in the same turn — don't leave open issues that are actually done.
...
- **Git is single-writer.** The working tree and index are shared mutable state. Don't run git mutations while a subagent is also touching the same checkout. Resolve merges/rebases inline yourself; if you must delegate git work to a subagent, give it an isolated worktree (`isolation: "worktree"`). Note that `isolation: "worktree"` branches from `origin/main` (not the caller's current branch HEAD) — pass an explicit `branch` arg if the worktree should continue from your feature branch instead.
...
- **Validate an external format/integration against its real downstream consumer, not a synthetic proxy.** A headless CLI validator can exercise a completely different code path than the GUI or app it's meant to feed (Bambu's slicer CLI uses a different loader than the desktop app's `load_project`, and OrcaSlicer is not a valid proxy for Bambu-specific behavior) — a "passes my harness" result isn't evidence until the harness is the actual consumer. For any export-format work, an early question is "can I run the real thing that opens this file," not just "can I round-trip it myself."
- **A delegated subagent's "done and validated" claim can be wrong — re-verify before relying on it.** This applies doubly to `isolation: "worktree"` work: confirm the worktree actually branched from the current feature HEAD (not a stale `origin/main`) before integrating its result.
