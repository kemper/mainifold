...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool doesn't expose a `-a` flag at all** (passing one is not supported) — don't spend a call finding that out; go straight to `Bash` with `grep -an` / `rg -a`.
...
If a grep on `main.ts` returns nothing for a symbol you expect to find there, binary-detection is the first thing to check. Four independent sessions have each spent turns re-discovering this (most recently: burning a call on the `Grep` tool's unsupported `-a` before falling back to `Bash`-grep).
