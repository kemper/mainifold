...
15. **STL import**: Click Import → "Choose file…" → pick an `.stl`. A new session is created named after the file, the editor shows a short `return Manifold.ofMesh(api.imports[0])` wrapper, and the mesh renders in the viewport. The version label is "imported" and editing the wrapper (e.g. adding `.subtract(Manifold.cube([5,5,5], true))`) re-renders correctly. Closing and reopening the session must restore the imported mesh from IndexedDB. Heavy meshes (> 20k triangles) first show an "Import detail" modal offering optional tolerance-based reduction (`simplify`); reduced imports get the label "imported · reduced" and a `[IMPORT]` session note. Imported meshes are exempt from the global "Mesh detail" refine factor (see the gear/Detail controls), so their triangle count is preserved on import.
