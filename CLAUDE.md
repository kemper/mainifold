...
12. **Color export**: With color regions painted, export GLB — the file should carry vertex colors. Export 3MF — the file should include an `<m:colorgroup>` and per-triangle `pid`/`p1` attributes. The export menu also offers **3MF (Bambu — all PLA)** (`export3MFBambu`), which writes the same mesh + colorgroup plus a `Metadata/project_settings.config` declaring one Generic PLA filament per color so Bambu Studio imports every color typed PLA instead of color-matching to a mix of PLA/ABS presets.
