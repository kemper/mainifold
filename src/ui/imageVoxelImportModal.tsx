...
function ImageVoxelBody(props: { image: ImageDataLike; filename: string; state: Signal<Opts> }) {
  ...
  return (
    <div class="flex flex-col gap-3">
      <p class="text-[11px] text-zinc-400 leading-relaxed">
        Import <span class="text-zinc-200 font-medium">{props.filename}</span> as voxels.
      </p>
...
function ImageVoxelFooter(props: {
  ...
  const empty = computeImageVoxelLayout(props.image, opts).voxelCount === 0;
  return (
    <>
...
export function showImageVoxelImportModal(opts: ImageVoxelModalOptions): Promise<ImageToVoxelOptions | null> {
  return new Promise(resolve => {
    ...
    mountPreactModal(
      ...
      close => ({
        body: <ImageVoxelBody image={opts.image} filename={opts.filename} state={state} />,
        footer: (
          <ImageVoxelFooter
            image={opts.image}
            state={state}
            onImport={o => { result = o; close(); }}
