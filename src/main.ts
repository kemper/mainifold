...
/** True when a descriptor drives mesh subdivision: any brush stroke, the
 *  airbrush, or a slab /
...
function descriptorRefines(d: RegionDescriptor): boolean {
  if (d.kind === 'brushStroke' || d.kind === 'airbrush') return true;
...
function collectRefineRegions(descriptors: RegionDescriptor[]): RefineRegion[] {
  ...
  for (const d of descriptors) {
    if (d.kind === 'brushStroke') {
      ...
      regions.push(airbrushRefineRegion(d.samples, d.radius, d.maxEdge > 0 ? d.maxEdge : d.radius / 10));
  ...
  switch (descriptor.kind) {
    ...
    case 'airbrush':
      return airbrushFootprintTriangles(mesh, {
        samples: descriptor.samples, radius: descriptor.radius,
        strength: descriptor.strength, softness: descriptor.softness,
        seed: descriptor.seed, maxEdge: descriptor.maxEdge > 0 ? descriptor.maxEdge : descriptor.radius / 10,
      });
  }
}
...
function appendStrokeRefine(descriptor: Extract<RegionDescriptor, { kind: 'brushStroke' | 'airbrush' }>): void {
  ...
  const region = descriptor.kind === 'airbrush'
    ? airbrushRefineRegion(descriptor.samples, descriptor.radius, descriptor.maxEdge > 0 ? descriptor.maxEdge : descriptor.radius / 10)
...
/** Stroke-style descriptors that drive the incremental refine-append fast path
 *  (brush strokes + airbrush sprays). */
function isStrokeDescriptor(d: RegionDescriptor): d is Extract<RegionDescriptor, { kind: 'brushStroke' | 'airbrush' }> {
...
function reconcilePaintedGeometry(): void {
  ...
  if (strokesNow.length === lastStrokeList.length + 1 && prefixRefEqual(strokesNow, lastStrokeList)) {
    const newDesc = strokesNow[strokesNow.length - 1] as Extract<RegionDescriptor, { kind: 'brushStroke' | 'airbrush' }>;
